    ...
    <div className={className} style={{ boxSizing: 'border-box', display: 'inline-block', transform: `scale(${scale})`, transformOrigin: 'top left', ...style }}>
      <div style={{ width: outerWidth, height: outerHeight, borderRadius: outerRadius, background: frameGradient, padding: resolvedBezel, boxSizing: 'border-box', boxShadow: outerShadow, position: 'relative', overflow: 'hidden', ...frameStyle }} aria-label={`iPhone mockup (${model})`}>
        <div style={{ width: '100%', height: '100%', borderRadius: resolvedRadius, position: 'relative', overflow: 'hidden', background: screenBg, boxShadow: innerShadowCss, ...screenStyle }}>
          ...
          {useIsland && finalIslandW > 0 && (<div aria-hidden style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', top: 12, width: finalIslandW, height: finalIslandH, borderRadius: finalIslandR, background: '#000', zIndex: 2, boxShadow: '0 1px 2px rgba(0,0,0,0.7)' }} />)}
          {!useIsland && useNotch && finalNotchW > 0 && (<div aria-hidden style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', top: 8, width: finalNotchW, height: finalNotchH, borderRadius: finalNotchR, background: '#000', zIndex: 2 }} />)}
          ...
          {showHomeIndicator && (<div aria-hidden style={{ position: 'absolute', bottom: 8, left: '50%', transform: 'translateX(-50%)', width: Math.round(screenWidth * 0.34), maxWidth: 140, height: 5, borderRadius: 3, background: 'linear-gradient(180deg, rgba(255,255,255,0.7), rgba(255,255,255,0.35))', opacity: 0.9, zIndex: 3, pointerEvents: 'none' }} />)}
        </div>
      </div>
    </div>
  );
